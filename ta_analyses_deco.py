...
# Apply DeMark Sequential and Combo
result = demark_sequential_and_combo(fake_data)
...
# Add combo numbers in blue
for i in range(len(result)):
    if result['combo'].iloc[i] != 0:
        fig.add_annotation(x=result.index[i], y=result['close'].iloc[i],
                           text=str(result['combo'].iloc[i]),
